...
def test_auto_fenster_liefert_gueltiges_band():
    ...
    Trifft polderfit.fit.autowindows.auto_fenster (nutzt np.ptp) – haette den
    NumPy-2.0-Regress 'ndarray has no attribute ptp' erwischt.
    """
    from polderfit.fit.autowindows import auto_fenster
...
def test_auto_fenster_alle_folgt_dispersion_auf_gekruemmtem_untergrund():
    ...
    from polderfit.fit.autowindows import auto_fenster_alle
...
def test_fenster_aus_trasse_zentriert_auf_vorgabe():
    ...
    from polderfit.io.datensatz import Messdatensatz
    from polderfit.fit.autowindows import fenster_aus_trasse
