...
def chi_ip_komponenten(
    ...
    Variante OHNE ``ratio`` (die Standard-ip-PolderFit-Konfiguration: Feld in der
    Ebene, senkrecht zum CPW-Innenleiter).

    HINWEIS – noch nicht im Linescan-Fit verdrahtet: Der Einzelfit
    (:func:`polderfit.physik.fitmodell.s21_modell`) verwendet derzeit
    ...
    :func:`polderfit.physik.kittel_llg.kittel_ip`) und einen ``geometrie``-Parameter
